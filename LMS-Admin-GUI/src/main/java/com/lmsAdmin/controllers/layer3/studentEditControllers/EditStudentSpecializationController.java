...
public class EditStudentSpecializationController extends MainFrameController {
    ...
    public void initialize() {
        ...
        idComboBox.setOnAction(event -> {
            curTextField.setText(manager.getOldSpecialization());
            EditSpecializationManager speManager = new EditSpecializationManager();
            newComboBox.getItems().addAll(speManager.loadIdsAndName());
    ...
    public void handleEdit() {
        if (isConfirmed()) {
            manager.manageEditSpecialization(extractId(newComboBox.getSelectionModel().getSelectedItem()));
